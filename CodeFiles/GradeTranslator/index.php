...
#$sql ="SELECT username,password FROM tb_login WHERE username=:username and password=:password and status=(:status)";
...
if($query->rowCount() > 0)
{
echo "<script type='text/javascript'> document.location = 'register_class.php'; </script>";
} else{
  
  echo "<script>alert('Login credentials are not correct');</script>";
